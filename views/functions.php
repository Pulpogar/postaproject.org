...
function obtener_proyecto_por_usuario($conexion, $autorSubida){
	$resultado = $conexion->query("SELECT * FROM proyectos WHERE autorSubida = '$autorSubida'");
	$resultado = $resultado->fetchAll();
	return ($resultado) ? $resultado : false;
}
...
function insertNewLog ($conexion, $idusuario, $username, $rol, $action){
	$flag=false;
	$idLog = obtener_max_id($conexion, 'logs', 'idlog');
    $statement = $conexion->prepare('INSERT INTO logs (idlog, idusuario, username, rol, actionLog, dateLog) VALUES (:idlog, :idusuario, :username, :rol, :actionLog, now())');
...
function obtener_usuarios($conexion){
	$resultado = $conexion->query("SELECT r.nombre as 'rol', u.* FROM usuarios u INNER JOIN roles r ON r.id = u.idrol");
	$resultado = $resultado->fetchAll();
	return ($resultado) ? $resultado : false;
}
